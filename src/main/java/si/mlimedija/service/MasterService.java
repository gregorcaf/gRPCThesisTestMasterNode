...
//        logger.info("PUT_DATA request: key=" + key + "|value=" + value + "|nodeId=" + nodeId + "|ipAddress=" + nodeIpAddress + "|port=" + nodePort);
    ...
    @Override
    public StreamObserver<PutFileRequest> putFile(StreamObserver<PutFileResponse> responseObserver) throws FileNotFoundException {
        return new StreamObserver<PutFileRequest>() {

            // location where we store streamed file
            Path filePath = Paths.get("/Users/gregorcaf/Desktop/Data/orders.tbl");
            FileOutputStream outputStream = new FileOutputStream(filePath.toFile());
            int chunkCount = 0;

            @Override
            public void onNext(PutFileRequest putFileRequest) {
                try {
                    outputStream.write(putFileRequest.getContent().toByteArray());
                    chunkCount++;
                    logger.info("Stream chunk: " + chunkCount);
                } catch (IOException e) {
            ...
            public void onError(Throwable throwable) {
                logger.error("File upload failed: " + throwable.getMessage());
                try {
                    if (outputStream != null) {
                        outputStream.close();
                    }
                } catch (IOException e) {
                    logger.error("Failed to close file output stream: " + e.getMessage());
                }
            }
            ...
            public void onCompleted() {
                ...
                PutFileResponse response = PutFileResponse.newBuilder()
                        .setResponseCode(200)
                        .setResponseMessage("File received in " + chunkCount + " chunks")
                        .build();

                // send response to the client
                responseObserver.onNext(response);

                // signal the completion of the response stream
                responseObserver.onCompleted();
            }
        };
    }
    ...
    @Override
    public void getFile(GetFileRequest request, StreamObserver<GetFileResponse> responseObserver) throws IOException {
        // extract file name from client's request
        String fileName = request.getFileName();
        Path filePath = Paths.get("/Users/gregorcaf/Desktop/Data/" + fileName);
        logger.info("File path: " + filePath);

        try {
            FileInputStream inputStream = new FileInputStream(filePath.toFile());

            byte[] buffer = new byte[4 * 1024 * 1024]; // 4 MB chunks
            int bytesRead;
            int chunkNumber = 1;
